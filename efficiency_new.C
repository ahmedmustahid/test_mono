...
void fill_hist(float *a, float *ctheta,float *costheta, float *tol, TH2F* h, TH2F* h1);
...
//  	data->SetBranchAddress("mcp_endz",mcp_endz);
   ...
   //     }
	...
	TH1F* he_nung= new TH1F("he_nung",";E",100,0,250);

	fill_hist(a,wt,ctheta,tol,h2,hwt);
...
void fill_hist(float *a, float *ctheta,float *costheta, float *tol, TH2F* h, TH2F* h1){
	...
	for(int i=0; i<nung_tot; i++){
		...
		if(!(get>2.0)  ){
			...
			h1->Fill(a[i],ctheta[i],costheta[i]);
			//cout<<"get==1 "<<get<<endl;
